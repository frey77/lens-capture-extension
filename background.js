...
async function startCaptureFlow (tab) {
  try {
    ...
    await notifyTab(tab.id, 'LENS_CAPTURE_STATUS', '正在截图…')
    ...
    await notifyTab(tab?.id, 'LENS_CAPTURE_ERROR', error.message || '截图失败')
  }
}
...
async function handleCroppedImage (croppedDataUrl, tab) {
  ...
  const target = await getDefaultTarget()
  await notifyTab(tab?.id, 'LENS_CAPTURE_STATUS', `正在打开${getTargetLabel(target)}…`)
  ...
  await notifyTab(tab?.id, 'LENS_CAPTURE_STATUS', `已在新标签页打开${getTargetLabel(target)}`)
}
...
async function injectImageIntoSearchPage (target, dataUrl) {
  const configMap = {
    baidu: {
      inputSelector: 'input[type="file"].general-upload-file, input[type="file"]',
      timeout: 15000
    },
    bing: {
...
async function attachImageToAiChat (target, dataUrl) {
  ...
  const inputSelectors = selectorMap[target] || selectorMap.chatgpt
  const triggerSelectors = triggerSelectorMap[target] || []
  const composerSelectors = composerSelectorMap[target] || composerSelectorMap.chatgpt
  ...
  while (Date.now() <= deadline) {
    const input = findMatchingElement(inputSelectors)
    if (input) {
      const transfer = new DataTransfer()
      transfer.items.add(file)
      input.files = transfer.files
  ...
  function findMatchingElement (selectors) {
    for (const selector of selectors) {
      const element = document.querySelector(selector)
